...
void main() {
  vec2 part1 = calculateCurve(p0, p0, p1, p2, t / 0.2);
  vec2 part2 = calculateCurve(p0, p1, p2, p3, (t - 0.2) / 0.6);
  vec2 part3 = calculateCurve(p1, p2, p3, p3, (t - 0.8) / 0.2);
  
  vec2 pos = part1 * float(t >= 0.0 && t < 0.2) +
             part2 * float(t >= 0.2 && t < 0.8) +
             part3 * float(t >= 0.8);
